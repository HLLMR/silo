fn main() {
    ...
    let gv = silo_lib::xmlconfig::get_values(
        &g,
        &[
    ...
    let sv = silo_lib::xmlconfig::get_values(
        &s,
        &[
    ...
    let out = silo_lib::xmlconfig::set_values(
        &s,
        &[silo_lib::xmlconfig::Edit {
            path: "careerSavegame.settings.weedsEnabled".into(),
            value: "false".into(),
        }],
    )
    .unwrap();
    println!(
        "set weeds=false ok: {}",
        out.contains("<weedsEnabled>false</weedsEnabled>")
    );
    println!(
        "timeScale preserved: {}",
        out.contains("<timeScale>10.000000</timeScale>")
    );
}
